...
public struct PickerView: UIViewRepresentable {
    ...
    public func makeUIView(context: UIViewRepresentableContext<PickerView>) -> UIPickerView {
        ...
        selectCurrentRows(in: picker, coordinator: context.coordinator)
    ...
    public func updateUIView(_ view: UIPickerView, context: UIViewRepresentableContext<PickerView>) {
        ...
        selectCurrentRows(in: view, coordinator: context.coordinator)
    }

    private func selectCurrentRows(in picker: UIPickerView, coordinator: Coordinator) {
        switch style {
            case .date, .time:
                coordinator.selectRow(
                    coordinator.row(date: selectedDate, component: 0),
                    inComponent: 0, in: picker, animated: false)
            case .dateRange, .timeRange:
                coordinator.selectRow(
                    coordinator.row(date: selectedRange.start, component: 0),
                    inComponent: 0, in: picker, animated: false)

                coordinator.selectRow(
                    coordinator.row(date: selectedRange.end, component: 1),
